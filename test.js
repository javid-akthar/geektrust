...
let {inBetweenCellsLength, requiredDirectionFinder, directionOptions, sameAxis,differentAxis, gManPower} = require("./g_man_power");
...
describe("Test Suite",function(){
    ...
    it("Test the requiredDirectionFinder method", function(){
        expect(requiredDirectionFinder(2, 4, 1, 1)).to.be.equal('E');
        expect(requiredDirectionFinder(0, 6, 6, 6)).to.be.equal('E');
        expect(requiredDirectionFinder(1, 5, 3, 3)).to.be.equal('E');
        expect(requiredDirectionFinder(5, 1, 3, 3)).to.be.equal('W');
    })
    it("Test the directionOptions method", function(){
        expect(directionOptions(2, 4, 1, 3)).to.deep.equalInAnyOrder({ option1: 'N', option2: 'E' })
        expect(directionOptions(0, 6, 5, 1)).to.deep.equalInAnyOrder({ option1: 'S', option2: 'E' })
        expect(directionOptions(3, 1, 6, 0)).to.deep.equalInAnyOrder({ option1: 'S', option2: 'W' })
        expect(directionOptions(2, 4, 1, 5)).to.deep.equalInAnyOrder({ option1: 'N', option2: 'E' })
    })
